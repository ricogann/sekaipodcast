...
class FollowInfoUseCase @Inject constructor(
    private val repository: ProfileRepository
) {
    suspend operator fun invoke(body: FollowInfo): Flow<Resource<ResponseFollowInfo>> = flow {
        try {
            emit(Resource.Loading<ResponseFollowInfo>())
            val response = repository.followInfo(body)
